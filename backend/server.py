...
async def create_reel(payload: ReelCreate):
    ...
    # mirror to planner as content event
...
async def _meera_reply(message: str, session_id: str) -> dict:
    ...
    chat = LlmChat(api_key=EMERGENT_LLM_KEY, session_id=session_id, system_message=MEERA_SYSTEM).with_model("anthropic", "claude-sonnet-4-5-20250929")
    ...
    raw = await chat.send_message(UserMessage(text=context + message))
...
async def reels_meera_suggestion():
    ...
    try:
        chat = LlmChat(api_key=EMERGENT_LLM_KEY, session_id="meera-reels", system_message=MEERA_SYSTEM).with_model("anthropic", "claude-sonnet-4-5-20250929")
        summary = (
        ...
        raw = await chat.send_message(UserMessage(text=summary))
        parsed = _parse_meera_json(raw)
...
async def meera_verdict():
    ...
    try:
        chat = LlmChat(api_key=EMERGENT_LLM_KEY, session_id="meera-verdict", system_message=MEERA_SYSTEM).with_model("anthropic", "claude-sonnet-4-5-20250929")
        prompt = (
